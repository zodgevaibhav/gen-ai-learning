...
text_df = pd.read_csv("case_paper.csv")
text = list(text_df.detailed_diagnosis_chikitsa.values)
...
print(f"len of unique tokens = {len(unique_tokens)}")
unique_tokens
...
for index, token in enumerate(unique_tokens):
    unique_token_index[token] = index
...
# Function to predict the next word(s) given an input sequence
# 'input_text' is the sequence of words, and 'n_best' is the number of top predictions to return
def predict_next_word(input_text, n_best):
    input_text = input_text.lower()  # Convert input text to lowercase for consistency
    X = np.zeros((1, n_words, len(unique_tokens)))  # Initialize input array for prediction
    for i, word in enumerate(input_text.split()):
        X[0, i, unique_token_index[word]] = 1  # One-hot encode the input words
        
    predictions = model.predict(X)[0]  # Get the probability distribution for the next word
    print(f"predictions = {predictions}")
    return np.argpartition(predictions, -n_best)[-n_best:]  # Return indices of top 'n_best' predictions

# Example usage of the prediction function
possible_words = predict_next_word("अमृता गु सत्व  यष्टि", 3)

# Print the predicted words corresponding to the top indices
for idx in possible_words:
